...
Transformar o Módulo 2 na fundação de decisão do curso: partir de uma oportunidade e produzir um Documento de Arquitetura de Software rastreável antes de escolher padrões, modelos ou fornecedores.
...
4. O Documento de Arquitetura de Software é construído progressivamente em conceitos, decisões, exemplo, caso, oficina, exercícios e síntese.
...
7. Ponto de vista define as convenções de representação; visão representa o sistema segundo essas convenções; modelos compõem visões; ADR registra uma decisão e cenário de qualidade especifica um requisito.
8. As visões mínimas cobrem contexto, responsabilidades, interação, informação e implantação. Regras de correspondência tornam contradições entre elas verificáveis.
...
- Declarar “Documento de Arquitetura de Software” como convenção didática do curso, não como termo normativo.

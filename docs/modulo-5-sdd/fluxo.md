...
Quatro abordagens para o mesmo padrão, o fluxo de oito etapas do Spec Kit em detalhe, três portões humanos e oito artefatos que precisam contar a mesma história sobre a mesma mudança.

Todo fluxo SDD resolve o mesmo problema: transformar uma intenção incompleta em software com evidência rastreável, sem que o agente decida sozinho o que a organização quer. As abordagens divergem em onde o contrato mora, quantos portões existem e quanto da disciplina é documental ou operacional. Esta página começa pelo panorama das quatro abordagens mais relevantes, e só então percorre uma delas, o [Spec Kit](https://github.com/github/spec-kit), etapa por etapa, como exemplo trabalhado.

## Quatro abordagens para o mesmo padrão

Quatro abordagens resolvem o mesmo problema, transformar intenção em evidência, com formas bem diferentes de organizar o trabalho. Conhecê-las antes de aprender os comandos de uma delas evita confundir a sintaxe de uma ferramenta com o princípio que ela serve.
...
Para tornar a comparação concreta, as quatro abordagens serão aplicadas à mesma tarefa hipotética: adicionar limitação de tentativas de login à API de um banco digital consumida por vários canais. O caso é diferente da exportação de avaliações usada mais adiante nesta página de propósito, porque comparar quatro abordagens exige um problema pequeno o suficiente para caber quatro vezes seguidas.

O pedido inicial, “bloquear a conta depois de cinco tentativas”, parece pequeno, mas esconde perguntas que qualquer fluxo precisa fazer antes de gerar código:
...
**Como funciona.** É o fio operacional desta disciplina, detalhado etapa por etapa [na seção seguinte](#o-fluxo-do-spec-kit-em-detalhe). Na documentação atual, o fluxo agêntico inclui nove comandos: constitution, especificação, esclarecimento, plano, checklist, tarefas, análise, implementação e convergência. Esclarecimento, checklist e análise funcionam como portões de qualidade acionados conforme a ambiguidade e o risco. A convergência procura requisitos ou decisões ainda não atendidos e acrescenta trabalho rastreável à lista de tarefas. A ferramenta também passou a oferecer extensões, presets e workflows para adaptar ou automatizar o processo (GitHub, 2026a, 2026b).
...
## O fluxo do Spec Kit em detalhe

Das quatro abordagens acima, o módulo percorre uma até o fim. A escolha do Spec Kit como exemplo trabalhado está registrada num [ADR](decisoes.md#adr-spec-kit-como-fio-operacional-nao-como-metodologia-exclusiva): ele nomeia cada etapa por um comando observável, o que permite praticar um fluxo concreto sem tratar a ferramenta como sinônimo do método.

O fluxo didático deste módulo usa:

`constitution → specify → clarify → plan → tasks → analyze → implement → verify`

Os nomes ficam em inglês porque é assim que você vai digitá-los. O que cada um significa está no [vocabulário mínimo](index.md#vocabulario-minimo) do módulo. Nenhuma etapa depende de ferramenta: o que importa é qual incerteza cada uma remove e qual artefato entrega.

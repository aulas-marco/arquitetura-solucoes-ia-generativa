...
**Caso contínuo — Banco Lume.** [← Módulo 5: Especificação](../modulo-5-sdd/caso-lume.md) · [Módulo 7: Operação →](../modulo-7-operacao/caso-lume.md)
...
Antes de executar, releia [Instruções adversariais](ameacas-e-guardrails.md#instrucoes-adversariais) — `L-01` é injeção direta pedindo para ignorar a política, `L-02` é fonte omissa que exige escalonamento por conflito — e [Exposição e efeitos indevidos](ameacas-e-guardrails.md#exposicao-e-efeitos-indevidos), que cobre o vazamento de contexto que `L-03` tenta provocar ao pedir o texto reservado do sistema. `L-04` e `L-05` testam a fronteira de aprovação humana do [registro de risco](#registro-de-risco) desta página: correção de rotina por evidência faltante e divergência entre analista e rascunho.

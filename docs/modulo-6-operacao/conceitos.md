...
Um trace distribuído conecta a solicitação às etapas de **prompt**, **contexto**, **recuperação**, **ferramenta** e **resposta**. Cada span registra tempo, resultado, versão, política aplicada e relação causal. O span de recuperação inclui consulta derivada, índice, filtros de autorização, quantidade e identificadores de documentos; o de ferramenta registra contrato, operação, decisão de política, idempotência e status; o de resposta registra validações e rota de entrega.

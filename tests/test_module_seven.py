...
MODULE = ROOT / "docs" / "modulo-7-operacao"
...
class ModuleSevenContentRegressionTest(unittest.TestCase):
    ...
    def test_module_has_standard_pages_navigation_and_guiding_question(self):
        ...
        ordem = [navigation.index(f"modulo-7-operacao/{page}") for page in ("index.md", "exemplo-arquitetural.md", "estudo-de-caso.md",
                     "oficina-de-ferramentas.md", "exercicios.md", "sintese-e-referencias.md")]
        self.assertEqual(ordem, sorted(ordem))
        for page in tematicas:
            self.assertIn(f"modulo-7-operacao/{page}", navigation)
    ...
    def test_architectural_example_has_images_diagrams_ownership_and_containment(self):
        ...
        for image in ("m07-ciclo-llmops.png", "m07-plataforma-corporativa.png"):

...
MODULE = ROOT / "docs" / "modulo-6-confianca"
...
class ModuleSixContentRegressionTest(unittest.TestCase):
    def test_module_has_standard_pages_navigation_and_guiding_question(self):
        ...
        ordem = [navigation.index(f"modulo-6-confianca/{page}") for page in ("index.md", "exemplo-arquitetural.md", "estudo-de-caso.md",
                     "oficina-de-ferramentas.md", "exercicios.md", "sintese-e-referencias.md")]
        self.assertEqual(ordem, sorted(ordem))
        for page in tematicas:
            self.assertIn(f"modulo-6-confianca/{page}", navigation)
    ...
    def test_architectural_example_has_images_diagrams_and_text_equivalents(self):
        ...
        for image in ("m06-defesas-profundidade.png", "m06-prisma-avaliacao.png"):
